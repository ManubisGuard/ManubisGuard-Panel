...
## Installer SSL wizard TTY fix — 2026-09-25
- [x] Fixed the interactive SSL wizard for the documented `curl | bash` installation form: all wizard `read` operations now read from `/dev/tty` instead of the piped script stdin.
...
- [x] Node Docker build gate passed after resolving the inherited controller client-isolation dependency; image manubisguard-node:multicore-test3 exported successfully and the Go compiler reached controller/rest, controller/rpc, and cmd/node without errors.
- [ ] Run the controller multi-core Go test suite and live Panel/Node multi-Core E2E check; the broad go test ./controller ./backend/... ./common attempt entered the existing backend integration suite and was stopped rather than leaving an unbounded test process.
