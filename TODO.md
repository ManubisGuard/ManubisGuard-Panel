...
- [x] Node Docker build gate passed after resolving the inherited controller client-isolation dependency;  exported successfully and the Go compiler reached , , and  without errors.
- [ ] Run the controller multi-core Go test suite and live Panel/Node multi-Core E2E check; the broad  attempt entered the existing backend integration suite and was stopped rather than leaving an unbounded test process.
