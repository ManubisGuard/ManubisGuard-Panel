...
stage "Seed source database" bash -c 'set -e; docker exec "$1" psql -U postgres -d source_db -v ON_ERROR_STOP=1 -c "CREATE EXTENSION IF NOT EXISTS timescaledb"; docker exec "$1" psql -U postgres -d source_db -v ON_ERROR_STOP=1 -c "CREATE TABLE public.devices (id integer PRIMARY KEY, name text NOT NULL)"; docker exec "$1" psql -U postgres -d source_db -v ON_ERROR_STOP=1 -c "CREATE TABLE public.usage (time timestamptz NOT NULL, device_id integer NOT NULL REFERENCES public.devices(id), bytes bigint NOT NULL)"; docker exec "$1" psql -U postgres -d source_db -v ON_ERROR_STOP=1 -c "SELECT create_hypertable('''public.usage''', by_range('''time''', INTERVAL '''1 day'''))"; docker exec "$1" psql -U postgres -d source_db -v ON_ERROR_STOP=1 -c "INSERT INTO public.devices SELECT g, '''device-''' || g FROM generate_series(1,3) g"; docker exec "$1" psql -U postgres -d source_db -v ON_ERROR_STOP=1 -c "INSERT INTO public.usage SELECT TIMESTAMPTZ '''2026-01-01''' + g * INTERVAL '''1 day''', ((g-1)%3)+1, g*100 FROM generate_series(1,48) g"; docker exec "$1" psql -U postgres -d source_db -v ON_ERROR_STOP=1 -c "CREATE MATERIALIZED VIEW public.daily_usage WITH (timescaledb.continuous) AS SELECT time_bucket('''1 day''',time) bucket,min(device_id) device_id,sum(bytes) bytes FROM public.usage GROUP BY bucket WITH NO DATA"; docker exec "$1" psql -U postgres -d source_db -v ON_ERROR_STOP=1 -c "CALL refresh_continuous_aggregate('''public.daily_usage''',NULL,NULL)"; docker exec "$1" psql -U postgres -d source_db -v ON_ERROR_STOP=1 -c "ANALYZE"' _ "$SOURCE_CONTAINER"
